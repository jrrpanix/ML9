...
def runModels(models, model_data_set, Nitr, pctTrain, ngram, tfid):
    results, prec,recall,f1, nonZeroC, trainPos, testPos=[],[],[],[], [], np.zeros(Nitr), np.zeros(Nitr)
    ...
    return results, trainPos, testPos, prec, recall, f1, NF, norm, nz , sz, nonZeroC
...
if __name__ == '__main__':
    ...
    if args.nb :
        ...
        models=[
            ("Logistic Lasso0.3",LogisticRegression(penalty='l1',solver=solver,max_iter=max_iter, C=0.3)),
            ("Logistic Lasso1.0",LogisticRegression(penalty='l1',solver=solver,max_iter=max_iter, C=1.0)),
            ("Logistic Lasso2.0",LogisticRegression(penalty='l1',solver=solver,max_iter=max_iter, C=2.0)),
            ("Logistic Lasso5.0",LogisticRegression(penalty='l1',solver=solver,max_iter=max_iter, C=5.0)),
            ("Logistic",LogisticRegression(solver=solver,max_iter=max_iter))            ]

    print("Determining Fed Action from minutes")
    print("%-20s %5s %5s %10s %10s %5s %8s %7s %10s %10s %-27s %6s %6s %6s %6s %6s %5s %5s %10s %8s %8s %8s %10s %6s %8s" % 
          ("Model Name", "NGram", "Niter", "mean(acc)", "std(acc)","N","PctTrain", "clean", "start", "end", "Data Sets", "TrainP", "TestP", "Prec", "Recall", "F1", "Stack", "Tfid","NF", "Norm", "Ratio", "Sparcity", "Size", "NZMtx", "PctNZC"))

    outputDF = []

    for ngram in ngrams:
        results, trainPos, testPos, prec, recall, f1, NF, norm, nz, sz, nonZeroC = runModels(models, data_set, Niter, pctTrain, ngram, tfid)
        sparcity = nz/sz
        ngramstr = str(ngram[0]) + ":" + str(ngram[1])
        for m, r, t, u, v, coeff_nz in zip(models, results, prec, recall, f1, nonZeroC):
            name, mu, s, precMu, recallMu, f1Mu , normR, NonZeroCoeff = m[0], np.mean(r), np.std(r), np.mean(t), np.mean(u), np.mean(v), norm/NF, np.mean(coeff_nz)/NF
            print("%-20s %5s %5s %10.4f %10.4f %5d %8.3f %7s %10s %10s %-27s %6.3f %6.3f %6.3f %6.3f %6.3f %5s %5s %10.0f %8.0f %8.6f %8.6f %10.0f %6.0f %8.6f" % 
                  (name, ngramstr, Niter, mu, s, N, pctTrain, cleanA, start, end, datasetlabel, np.mean(trainPos), np.mean(testPos), precMu, recallMu, f1Mu, stack, tfidstr, NF, norm, normR, sparcity, sz, nz, NonZeroCoeff))

            outputDF.append([name, ngramstr, Niter, mu, s, N, pctTrain, cleanA, start, end, datasetlabel, np.mean(trainPos), np.mean(testPos), precMu, recallMu, f1Mu, stack, tfidstr, NF, norm, normR, sparcity, sz, nz, NonZeroCoeff])

    outputDF = pd.DataFrame(outputDF)
    print("writing to {}".format(args.output))
    outputDF.to_csv(args.output, index=False, header=["Model Name", "NGram", "Niter", "mean(acc)", "std(acc)","N","PctTrain", "clean", "start", "end", "Data Sets", "TrainP", "TestP", "Prec", "Recall", "F1", "Stack", "Tfid", "NF", "norm", "normR", "sparcity", "sz", "nz", "NonZeroCoeff"])
