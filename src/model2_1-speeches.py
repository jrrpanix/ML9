...
def splitTrainTest(data, data_statements, data_speeches, trainPct):
    ...
    test_data = test_data.append(pd.DataFrame(data_speeches,columns=['text','sentiment']))
...
def runModels(models, data, data_statements, data_speeches, Nitr, pctTrain, ngram):
    results=[]
    for iter in range(Nitr):
        train_data, test_data = splitTrainTest(data, data_statements, data_speeches, pctTrain)
